...
fn main() -> anyhow::Result<()> {
    ...
    match rt.block_on(client.query_positions()) {
        Ok(positions) => {
            ...
            for pos in &positions {
                let dir = pos.posi_direction.as_deref().unwrap_or("N/A");
                let inst = pos.instrument_id.as_deref().unwrap_or("N/A");
                let pos_size = pos.position.as_deref().unwrap_or("N/A");
                output.push_str(&format!("  - {} 方向:{} 数量:{}\n", inst, dir, pos_size));
            }
        }
        Err(e) => {
            output.push_str(&format!("❌ 失败: {}\n", e));
            tracing::error!("Step 1 失败: {:#?}", e);
        }
    }
    output.push_str("\n");
    ...
    match rt.block_on(client.market_open(SYMBOL, TradeDirection::Long, volume)) {
        Ok(resp) => {
            ...
            output.push_str(&format!("  成交价: {}\n", resp.trade_price));
            output.push_str(&format!("  开仓价: {}\n", resp.open_price));
            output.push_str(&format!("  状态: {}\n", resp.order_status));
        }
        Err(e) => {
            output.push_str(&format!("❌ 失败: {}\n", e));
            tracing::error!("Step 2 失败: {:#?}", e);
        }
    }
    output.push_str("\n");
    ...
    match rt.block_on(client.query_positions()) {
        Ok(positions) => {
            ...
            for pos in &positions {
                ...
                let inst = pos.instrument_id.as_deref().unwrap_or("N/A");
                let pos_size = pos.position.as_deref().unwrap_or("N/A");
                let open = pos.open_price.as_deref().unwrap_or("N/A");
    ...
    let trade_unit_id_result: anyhow::Result<Option<String>> = (|| {
        let positions = rt.block_on(client.query_positions())?;
        Ok(positions.into_iter()
            .find(|p| p.posi_direction.as_deref() == Some("0"))
            .and_then(|p| p.trade_unit_id))
    })();

    match trade_unit_id_result {
        Ok(Some(tid)) => {
            output.push_str(&format!("  TradeUnitID: {}\n", tid));
        ...
        Err(e) => {
            output.push_str(&format!("❌ 查询持仓失败: {}\n", e));
            tracing::error!("Step 4 query failed: {:#?}", e);
        }
    }
    output.push_str("\n");
    ...
    let short_position_result: anyhow::Result<Option<exchange_adapter_lbank::protocol::PositionResponse>> = (|| {
        let positions = rt.block_on(client.query_positions())?;
        Ok(positions.into_iter()
            .find(|p| p.posi_direction.as_deref() == Some("1")))
    })();

    match short_position_result {
        Ok(Some(pos)) => {
            let trade_unit_id = pos.trade_unit_id.as_deref().unwrap_or("");
        ...
        Err(e) => {
            output.push_str(&format!("❌ 查询持仓失败: {}\n", e));
            tracing::error!("Step 9 query failed: {:#?}", e);
        }
    }
    output.push_str("\n");
